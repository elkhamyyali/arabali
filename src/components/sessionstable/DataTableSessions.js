...
const DataTableSessions = ({ data }) => {
        ...
        {columns.map((column) => (
          ...
          >
            {column.uid === "limitedRepeatance" ? (
              ...
              <span className={`p-1 rounded ${getColorClass(row[column.uid])}`}>
                {" "}
                {row[column.uid]}{" "}
              </span>
            ) : (
  ...
  const getColorClass = (value) => {
    switch (value) {
      case "Premium":
        return "bg-blue-200 text-blue-800 rounded-full px-3 text-xs";
      case "Burn":
        return "bg-red-200 text-red-800 rounded-full px-3 text-xs";
      case "Top":
        return "bg-green-200 text-green-800 rounded-full px-3 text-xs";
      case "Latest":
        return "bg-yellow-200 text-yellow-800 rounded-full px-3 text-xs";
      case "Pending":
        return "bg-gray-200 text-gray-800 rounded-full px-3 text-xs";
      case "Start":
        return "bg-blue-200 text-blue-800 rounded-full px-3 text-xs";
      case "Started":
        return "bg-green-200 text-green-800 rounded-full px-3 text-xs";
